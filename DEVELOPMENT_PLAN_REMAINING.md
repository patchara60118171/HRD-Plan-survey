...
> วันที่อัปเดตล่าสุด: 2026-03-22
> สถานะ: อัปเดตความคืบหน้าแล้ว (ความคืบหน้ารวม ~69%)
> เวอร์ชัน: 1.2

---

## 0. อัปเดตล่าสุด (Delta) ณ 2026-03-22

### งานที่เสร็จเพิ่มแล้ว (รอบนี้)

| งานที่ทำเพิ่ม | สถานะ | เหตุผลที่ทำ |
|--------------|-------|------------|
| Sprint 3C — แยกหน้า Admin เป็น page modules 7 ไฟล์ | ✅ เสร็จแล้ว | ลดความเสี่ยงจากไฟล์ monolithic |
| เพิ่ม `<script>` สำหรับ page modules ใน `admin.html` | ✅ เสร็จแล้ว | ให้ระบบโหลดแบบแยกโมดูล |
| **Org HR Scoping Fix** — `users.js` + `data.js` (Frontend) | ✅ เสร็จแล้ว | แก้ org_hr เห็นข้อมูลทุกองค์กร |
| **Critical Bug Fix** — Backfill `org_code` NULL/uppercase ใน `hrd_ch1_responses` | ✅ เสร็จแล้ว | RLS ไม่ match เพราะ org_code = NULL/uppercase (Issue #5) |
| **Prompt H** — Hardening `initial_password`: VIEW + RPC + REVOKE anon | ✅ เสร็จแล้ว | ป้องกัน org_hr/viewer เข้าถึง credentials ผ่าน direct query |
| **Prompt G** — Full Verification: ผ่านทุก 6 จุด | ✅ เสร็จแล้ว | ยืนยัน backend พร้อม production |

### หมายเหตุสำคัญ

- **COWORK_PROMPTS.md Prompt A-H ปิดครบทั้งหมดแล้ว** (2026-03-22)
- งานที่เหลือเป็น Frontend Dev tasks และ Sprint ถัดไป
- **Dev Action Pending:** เปลี่ยน query `admin_user_roles select('*')` → `admin_user_roles_public` ใน frontend
...
| 7  | สร้าง username/password 15 หน่วยงาน (org_hr)                  | กำลังพัฒนา (เหลือยืนยัน login) | 1      | **สูงมาก**  |
| 8  | แยก admin ออกเป็นโมดูล                                      | เสร็จแล้วบางส่วน (3A/3B/3C)         | 3      | สูง                  |
...
### เปอร์เซ็นต์ความคืบหน้า (อัปเดตล่าสุด)

เกณฑ์คำนวณจากสถานะในตารางด้านบน:
...
| มิติ                                  | จำนวนงาน | ความคืบหน้า |
| ----------------------------------------- | ---------------: | ---------------------: |
| รวมทั้งแผน (13 งาน)          |               13 |        **32.7%** |
| Sprint 1 (งาน #1, #4, #7)              |                3 |        **50.0%** |
| Sprint 2 (งาน #2, #3, #5, #6, #9, #10) |                6 |        **33.3%** |
| Sprint 3 (งาน #8)                      |                1 |        **75.0%** |
| Sprint 4 (งาน #11, #12)                |                2 |         **0.0%** |
| Sprint 5 (งาน #13)                     |                1 |         **0.0%** |

> หมายเหตุ: ตัวเลขนี้สะท้อน "ความคืบหน้าตาม Initiative ในเอกสารแผน" ไม่ใช่จำนวน task ย่อยทั้งหมดใน `TASK_TRACKER.md`

---

## Sprint 1 — ระบบฐานข้อมูลและ Backend (สัปดาห์ที่ 1-2)
...
- Username/Password ของ org_hr ต้องเก็บหลังบ้านใน `admin_user_roles.initial_password`
- หน้าดู credentials ให้เห็นได้เฉพาะ `admin` และ `superadmin`
- `org_hr` และ `viewer` ต้องไม่เห็น credentials ขององค์กรอื่น
- เมื่อมีการเปลี่ยนรหัสผ่าน org_hr โดยผู้ดูแล ระบบต้องอัปเดต `initial_password` ตามรหัสล่าสุด
...
## Sprint 2 — Admin UI Features (สัปดาห์ที่ 3-4)
...
## Sprint 3 — ปรับโครงสร้าง Admin Portal (สัปดาห์ที่ 5-6)

### 3.1 แยก admin ออกจากไฟล์เดียวเป็นโมดูล

**สถานะปัจจุบัน:** ทำเสร็จแล้วในระดับโครงสร้างหลัก (3A + 3B + 3C) แต่ยังมี inline script เดิมคงอยู่บางส่วนเพื่อรักษา backward compatibility ระหว่างช่วงเปลี่ยนผ่าน
...
| #  | ไฟล์ใหม่                    | หน้าที่                    |
| -- | ----------------------------------- | --------------------------------- |
| 8k | `admin/js/pages/dashboard.js`     | หน้า Dashboard                |
| 8l | `admin/js/pages/users.js`         | หน้า จัดการผู้ใช้ |
| 8m | `admin/js/pages/organizations.js` | หน้า องค์กร             |
| 8n | `admin/js/pages/links.js`         | หน้า ลิงก์               |
| 8o | `admin/js/pages/wellbeing.js`     | หน้า Well-being data          |
| 8p | `admin/js/pages/ch1.js`           | หน้า CH1 data                 |
| 8q | `admin/js/pages/settings.js`      | หน้า ตั้งค่า           |

**สถานะ Phase 3C ณ 2026-03-22:**

- ✅ สร้าง page files ครบ 7 ไฟล์
- ✅ เติม implementation หลักที่เคยเป็น TODO ใน `ch1.js`, `organizations.js`, `wellbeing.js`, `links.js`
- ✅ เพิ่ม script tags ใน `admin.html` ให้โหลด page modules แล้ว
- ⏳ งานคงค้างเชิงโครงสร้าง: ลดขนาด `admin.html` เพิ่มเติมโดยทยอยเอา inline logic ที่ซ้ำออกอย่างปลอดภัย
...
## Sprint 4 — ความปลอดภัยและทดสอบ (สัปดาห์ที่ 7-8)
...
## Sprint 5 — สถาปัตยกรรม Multi-app + Mobile (สัปดาห์ที่ 9+)
...
## Prompt สำหรับ Cowork (Supabase + Vercel)
