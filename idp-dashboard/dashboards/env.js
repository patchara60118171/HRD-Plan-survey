...
const genEmployee = (name, idx) => {
  ...
  // ข้อ 94: เคยได้ยิน "โรคอุบัติใหม่" หรือไม่ (true/false)
  const heardEmerging = rand(0, 4) > 0; // 80% เคยได้ยิน

  // ข้อ 95: โรคที่รู้จัก (multi-select)
  const knowsCovid = heardEmerging && rand(0, 3) > 0;
  const knowsBirdFlu = heardEmerging && rand(0, 2) > 0;
  const knowsZika = heardEmerging && rand(0, 3) === 0;
  const knowsOther = heardEmerging && rand(0, 4) === 0;

  // ข้อ 96: ผลกระทบจากสภาพภูมิอากาศ 0-4
  const climateImpact = pick([0, 0, 1, 1, 2, 2, 2, 3, 3, 4]);

  // ข้อ 97: ติด COVID-19 ช่วง 6 เดือน (0=ไม่เคย, 1=ติด1ครั้ง, 2=ติดซ้ำ)
  const covidHistory = pick([0, 0, 0, 0, 1, 1, 1, 2]);
...
// ─── Real data helpers ────────────────────────────────────────────────────────
function classifyEnv(emp) {
  const raw = emp._raw || emp;
  // Map real hazard fields (q85-q90): "ใช่ มีผลต่อสุขภาพ"=2, "ใช่ ไม่มีผล"=1, else=0
  const parseHaz = v => {
    if (v == null) return 0;
    const s = String(v);
    if (s.includes('มีผล') || s.includes('มีผลต่อ')) return 2;
    if (s.includes('ใช่') || s === '1') return 1;
    return 0;
  };
  const hazards = {
    sunlight: parseHaz(raw.q85 || raw.env_sunlight),
    noise: parseHaz(raw.q86 || raw.env_noise),
    chemical: parseHaz(raw.q87 || raw.env_chemical),
    fume: parseHaz(raw.q88 || raw.env_fume),
    posture: parseHaz(raw.q89 || raw.env_posture),
    awkward: parseHaz(raw.q90 || raw.env_awkward)
  };
  const hazardCount = Object.values(hazards).filter(v => v === 2).length;
  const parseScale = (v, max) => {
    const n = Number(v);
    return Number.isFinite(n) && n >= 0 && n <= max ? n : null;
  };
  const pm25Level = parseScale(raw.q91 ?? raw.env_pm25, 4) ?? pick([0, 0, 1, 1, 2, 2, 3, 4]);
  const symptoms = {
    none: String(raw.q92 || '').includes('ไม่มี'),
    cough: String(raw.q92 || '').includes('ไอ') || String(raw.q92 || '').includes('คัดจมูก'),
    breath: String(raw.q92 || '').includes('หายใจ'),
    eye: String(raw.q92 || '').includes('แสบตา'),
    headache: String(raw.q92 || '').includes('ปวดศีรษะ')
  };
  const symptomCount = Object.entries(symptoms).filter(([k, v]) => k !== "none" && v).length;
  const envSatisfaction = parseScale(raw.q84 ?? raw.env_satisfaction, 4) ?? rand(0, 4);
  const qualityOfLife = parseScale(raw.q93 ?? raw.env_qol, 4) ?? rand(0, 4);
  const climateImpact = parseScale(raw.q96 ?? raw.env_climate, 4) ?? pick([0, 0, 1, 1, 2, 2, 2, 3, 3, 4]);
  const cvStr = String(raw.q97 ?? raw.env_covid ?? '');
  const covidHistory = cvStr.includes('มากกว่า') || cvStr.includes('ซ้ำ') ? 2 : cvStr.includes('ติด') || cvStr === '1' ? 1 : 0;
  const heardEmerging = String(raw.q94 ?? '').includes('เคย') || raw.q94 === true;
  const pmRisk = pm25Level >= 3 && symptomCount > 0;
  const envRiskScore = hazardCount + (pmRisk ? 1 : 0);
  return {
    ...emp,
    id: emp.id || emp._raw?.id,
    name: emp.name || emp._raw?.name || '—',
    ...
    covidHistory,
    heardEmerging,
    knowsCovid: String(raw.q95 || '').includes('COVID'),
    knowsBirdFlu: String(raw.q95 || '').includes('หวัดนก'),
    knowsZika: String(raw.q95 || '').includes('ซิกา'),
    knowsOther: String(raw.q95 || '').includes('อื่น'),
    envRiskScore,
    envGroup: envRiskScore >= 2 ? "high" : envRiskScore >= 1 ? "medium" : "low"
  };
}
const _realEnv = typeof window !== 'undefined' && window.__IDP_EMPLOYEES__ && window.__IDP_EMPLOYEES__.env || null;
const employees = _realEnv && _realEnv.length > 0 ? _realEnv.map(e => classifyEnv(e)) : NAMES.map((n, i) => genEmployee(n, i));
...
function EnvDashboard() {
  ...
  return /*#__PURE__*/React.createElement("div", {
  ...
  }, "NIDA \xB7 ", employees.length, " \u0E04\u0E19 \xB7 \u0E02\u0E49\u0E2D 84\u201397 ", _realEnv ? "· ข้อมูลจริงจาก Supabase" : "· Mock Data")), /*#__PURE__*/React.createElement("div", {
    style: {
      display: "flex",
      gap: 10
    }
    ...
    const PM_LVL_LABELS = ["ไม่มี", "น้อย", "ปานกลาง", "มาก", "รุนแรงมาก"];
    const SYMP_ITEMS = [{
      key: "cough",
      label: "ไอ คัดจมูก แสบคอ"
    }, {
      key: "breath",
      label: "หายใจไม่เต็มอิ่ม"
    }, {
    ...
    const PM_HEAT_COLORS = ["#10B981", "#84CC16", "#F59E0B", "#F97316", "#EF4444"];
    const cellPct = (pmLvl, sympKey) => {
      const inGroup = employees.filter(e => e.pm25Level === pmLvl);
      if (!inGroup.length) return 0;
      return Math.round(inGroup.filter(e => e.symptoms[sympKey]).length / inGroup.length * 100);
    };
    return /*#__PURE__*/React.createElement("div", {
      style: {
        background: "#fff",
    ...
    }, "\uD83D\uDD2C PM2.5 \xD7 \u0E2D\u0E32\u0E01\u0E32\u0E23 Heatmap (\u0E02\u0E49\u0E2D 91\u201392)"), /*#__PURE__*/React.createElement("div", {
      style: {
        fontSize: 11,
        color: "#9CA3AF"
      }
    }, "% \u0E04\u0E19\u0E17\u0E35\u0E48\u0E21\u0E35\u0E2D\u0E32\u0E01\u0E32\u0E23 \u0E43\u0E19\u0E41\u0E15\u0E48\u0E25\u0E30\u0E23\u0E30\u0E14\u0E31\u0E1A PM2.5 \xB7 \u0E40\u0E0B\u0E25\u0E25\u0E4C\u0E40\u0E02\u0E49\u0E21\u0E02\u0E36\u0E49\u0E19 = \u0E40\u0E2A\u0E35\u0E48\u0E22\u0E07\u0E2A\u0E39\u0E07\u0E02\u0E36\u0E49\u0E19"))), /*#__PURE__*/React.createElement("div", {
      style: {
        overflowX: "auto"
      }
    ...
    }, "\u0E2D\u0E32\u0E01\u0E32\u0E23"), PM_LVL_LABELS.map((lbl, i) => {
      const cnt = employees.filter(e => e.pm25Level === i).length;
      return /*#__PURE__*/React.createElement("th", {
        ...
        style: {
          ...
          fontSize: 11,
          fontWeight: 700,
          color: PM_HEAT_COLORS[i],
          textAlign: "center",
          borderBottom: "2px solid #E5E7EB"
        }
      }, /*#__PURE__*/React.createElement("div", null, lbl), /*#__PURE__*/React.createElement("div", {
        style: {
          fontSize: 9,
          color: "#9CA3AF",
          fontWeight: 400
    ...
    }))), /*#__PURE__*/React.createElement("tbody", null, SYMP_ITEMS.map((s, si) => /*#__PURE__*/React.createElement("tr", {
      key: s.key,
      style: {
    ...
    }, s.label), [0, 1, 2, 3, 4].map(pmLvl => {
      const p = cellPct(pmLvl, s.key);
      const alpha = p < 25 ? 0 : p < 50 ? 0.20 : p < 75 ? 0.50 : 0.80;
      const bg = alpha === 0 ? "transparent" : `rgba(239,68,68,${alpha})`;
      return /*#__PURE__*/React.createElement("td", {
        ...
        style: {
          ...
          fontSize: 12,
          background: bg,
          fontWeight: p >= 50 ? 700 : 400,
          color: p >= 75 ? "#fff" : p >= 25 ? "#1E293B" : "#9CA3AF",
    ...
    }, [["< 25%", 0], ["25–50%", 0.20], ["50–75%", 0.50], ["75%+", 0.80]].map(([lbl, a]) => /*#__PURE__*/React.createElement("span", {
      key: lbl,
      style: {
        display: "flex",
        alignItems: "center",
      ...
      style: {
        ...
        height: 10,
        borderRadius: 2,
        background: a === 0 ? "#E5E7EB" : `rgba(239,68,68,${a})`,
    ...
    const CLIMATE_LABELS = ["ไม่มี", "น้อย", "ปานกลาง", "มาก", "รุนแรงมาก"];
    const CLIMATE_COLORS = ["#10B981", "#84CC16", "#F59E0B", "#F97316", "#EF4444"];
    const highImpact = employees.filter(e => e.climateImpact >= 3).length;
    const avgImpact = (employees.reduce((s, e) => s + e.climateImpact, 0) / employees.length).toFixed(1);
    return /*#__PURE__*/React.createElement("div", {
      style: {
        background: "#fff",
        borderRadius: 16,
    ...
    }, "\uD83C\uDF21\uFE0F \u0E1C\u0E25\u0E01\u0E23\u0E30\u0E17\u0E1A\u0E2A\u0E20\u0E32\u0E1E\u0E20\u0E39\u0E21\u0E34\u0E2D\u0E32\u0E01\u0E32\u0E28 (\u0E02\u0E49\u0E2D 96)"), /*#__PURE__*/React.createElement("div", {
      style: {
        fontSize: 11,
        color: "#9CA3AF"
      }
    }, "\u0E23\u0E30\u0E14\u0E31\u0E1A\u0E17\u0E35\u0E48\u0E23\u0E39\u0E49\u0E2A\u0E36\u0E01\u0E27\u0E48\u0E32\u0E2A\u0E20\u0E32\u0E1E\u0E2D\u0E32\u0E01\u0E32\u0E28\u0E2A\u0E48\u0E07\u0E1C\u0E25\u0E15\u0E48\u0E2D\u0E2A\u0E38\u0E02\u0E20\u0E32\u0E1E"))), /*#__PURE__*/React.createElement("div", {
      style: {
        display: "grid",
        gridTemplateColumns: "1fr 1fr",
        gap: 12,
    ...
    }, highImpact), /*#__PURE__*/React.createElement("div", {
      style: {
        fontSize: 11,
        color: "#6B7280"
      }
    }, "\u0E04\u0E19 \u0E23\u0E39\u0E49\u0E2A\u0E36\u0E01\u0E1C\u0E25\u0E01\u0E23\u0E30\u0E17\u0E1A \u0E21\u0E32\u0E01\u2013\u0E23\u0E38\u0E19\u0E41\u0E23\u0E07"), /*#__PURE__*/React.createElement("div", {
      style: {
        fontSize: 10,
        color: "#9CA3AF"
      }
    }, Math.round(highImpact / employees.length * 100), "% \u0E02\u0E2D\u0E07\u0E1A\u0E38\u0E04\u0E25\u0E32\u0E01\u0E23")), /*#__PURE__*/React.createElement("div", {
      style: {
        background: "#F0FDF4",
        borderRadius: 12,
        padding: "14px 16px",
    ...
    }, avgImpact), /*#__PURE__*/React.createElement("div", {
      style: {
        fontSize: 11,
        color: "#6B7280"
      }
    }, "\u0E04\u0E30\u0E41\u0E19\u0E19\u0E40\u0E09\u0E25\u0E35\u0E48\u0E22 (0\u20134)"), /*#__PURE__*/React.createElement("div", {
      style: {
        fontSize: 10,
        color: "#9CA3AF"
      }
    }, "\u0E40\u0E01\u0E13\u0E11\u0E4C\u0E40\u0E1D\u0E49\u0E32\u0E23\u0E30\u0E27\u0E31\u0E07 \u2265 3"))), CLIMATE_LABELS.map((lbl, i) => {
      const cnt = employees.filter(e => e.climateImpact === i).length;
      return /*#__PURE__*/React.createElement("div", {
        key: i,
        style: {
          marginBottom: 8
        ...
        style: {
          fontSize: 12,
          fontWeight: 700,
          color: CLIMATE_COLORS[i]
        }
      }, cnt, " \u0E04\u0E19 (", pct(cnt, employees.length), "%)")), /*#__PURE__*/React.createElement("div", {
        style: {
          height: 8,
          background: "#F3F4F6",
          borderRadius: 4
        ...
        style: {
          width: `${pct(cnt, employees.length)}%`,
          height: "100%",
          background: CLIMATE_COLORS[i],
          borderRadius: 4,
    ...
    }, "\uD83D\uDCA1 IDP: \u0E2A\u0E23\u0E49\u0E32\u0E07\u0E04\u0E27\u0E32\u0E21\u0E15\u0E23\u0E30\u0E2B\u0E19\u0E31\u0E01\u0E14\u0E49\u0E32\u0E19\u0E2A\u0E38\u0E02\u0E20\u0E32\u0E1E\u0E01\u0E31\u0E1A\u0E2A\u0E20\u0E32\u0E1E\u0E20\u0E39\u0E21\u0E34\u0E2D\u0E32\u0E01\u0E32\u0E28 / \u0E15\u0E34\u0E14\u0E15\u0E32\u0E21 AQI"));
  })(), (() => {
    const heardCount = employees.filter(e => e.heardEmerging).length;
    const covidNever = employees.filter(e => e.covidHistory === 0).length;
    const covidOnce = employees.filter(e => e.covidHistory === 1).length;
    const covidRepeat = employees.filter(e => e.covidHistory === 2).length;
    const diseaseKnowledge = [{
      label: "COVID-19",
      count: employees.filter(e => e.knowsCovid).length,
      color: "#EF4444"
    }, {
      label: "ไข้หวัดนก",
      count: employees.filter(e => e.knowsBirdFlu).length,
      color: "#F97316"
    }, {
      label: "ไข้ซิกา",
      count: employees.filter(e => e.knowsZika).length,
      color: "#F59E0B"
    }, {
      label: "อื่นๆ",
      count: employees.filter(e => e.knowsOther).length,
      color: "#6366F1"
    }];
    const covidData = [{
      name: "ไม่เคยติด",
    ...
    return /*#__PURE__*/React.createElement("div", {
    ...
    }, "\uD83E\uDDA0 COVID-19 + \u0E42\u0E23\u0E04\u0E2D\u0E38\u0E1A\u0E31\u0E15\u0E34\u0E43\u0E2B\u0E21\u0E48 (\u0E02\u0E49\u0E2D 94\u201397)"), /*#__PURE__*/React.createElement("div", {
      style: {
        fontSize: 11,
        color: "#9CA3AF"
      }
    }, "\u0E01\u0E32\u0E23\u0E23\u0E31\u0E1A\u0E23\u0E39\u0E49 + \u0E1B\u0E23\u0E30\u0E27\u0E31\u0E15\u0E34\u0E01\u0E32\u0E23\u0E15\u0E34\u0E14\u0E40\u0E0A\u0E37\u0E49\u0E2D"))), /*#__PURE__*/React.createElement("div", {
      style: {
        background: "#EEF2FF",
        borderRadius: 10,
        padding: "10px 14px",
    ...
    }, Math.round(heardCount / employees.length * 100), "%"), /*#__PURE__*/React.createElement("div", {
      style: {
        fontSize: 10,
        color: "#6366F1"
      }
    }, "\u0E23\u0E39\u0E49\u0E08\u0E31\u0E01 \"\u0E42\u0E23\u0E04\u0E2D\u0E38\u0E1A\u0E31\u0E15\u0E34\u0E43\u0E2B\u0E21\u0E48\"")), /*#__PURE__*/React.createElement("div", {
      style: {
        flex: 1
      }
    }, /*#__PURE__*/React.createElement("div", {
    ...
    }, "\u0E42\u0E23\u0E04\u0E17\u0E35\u0E48\u0E23\u0E39\u0E49\u0E08\u0E31\u0E01 (\u0E02\u0E49\u0E2D 95)"), diseaseKnowledge.map((d, i) => /*#__PURE__*/React.createElement("div", {
      key: i,
      style: {
        display: "flex",
        alignItems: "center",
      ...
      style: {
        width: `${pct(d.count, employees.length)}%`,
        height: "100%",
        background: d.color
      }
    })), /*#__PURE__*/React.createElement("span", {
    ...
    }, "\u0E1B\u0E23\u0E30\u0E27\u0E31\u0E15\u0E34\u0E15\u0E34\u0E14 COVID-19 (6 \u0E40\u0E14\u0E37\u0E2D\u0E19 \xB7 \u0E02\u0E49\u0E2D 97)"), /*#__PURE__*/React.createElement("div", {
      style: {
        display: "flex",
        gap: 16,
        alignItems: "center"
      ...
      outerRadius: 50,
      paddingAngle: 2
    }, covidData.map((d, i) => /*#__PURE__*/React.createElement(Cell, {
      key: i,
      fill: d.color
    }))), /*#__PURE__*/React.createElement(Tooltip, {
    ...
    }, covidData.map((d, i) => /*#__PURE__*/React.createElement("div", {
      key: i,
      style: {
        marginBottom: 8
      }
    ...
    }, d.value, " \u0E04\u0E19 (", pct(d.value, employees.length), "%)")), /*#__PURE__*/React.createElement("div", {
      style: {
        height: 5,
        background: "#F3F4F6",
        borderRadius: 3
    ...
    })))), covidRepeat > 0 && /*#__PURE__*/React.createElement("div", {
      style: {
        background: "#FEF2F2",
        borderRadius: 6,
        padding: "6px 8px",
    ...
    }, "\u26A0\uFE0F \u0E15\u0E34\u0E14\u0E0B\u0E49\u0E33 ", covidRepeat, " \u0E04\u0E19 \u2014 \u0E04\u0E27\u0E23\u0E15\u0E34\u0E14\u0E15\u0E32\u0E21\u0E20\u0E39\u0E21\u0E34\u0E04\u0E38\u0E49\u0E21\u0E01\u0E31\u0E19"))));
