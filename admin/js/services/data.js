...
const SURVEY_SELECT_FIELDS =
    'id,email,name,title,organization,gender,age,org_type,job,job_duration,bmi,bmi_category,is_draft,submitted_at,timestamp,tmhi_score';
...
const CH1_LITE_FIELDS =
    'id,org_code,organization,status,created_at,last_saved_at,submitted_at,form_version';
...
function getOrgCatalog() {
    ...
    return ADMIN_CANONICAL_ORGS.map(org => {
        const row =
            profileMap.get(org.code) ||
            state.orgProfiles.find(
                item => (item.org_name_th || item.display_name || '') === org.name
            ) ||
            {};
        return {
            name: org.name,
            ...
            contactPhone:
                row.coordinator_contact_line || row.settings?.coordinator_contact_line || '',
            contactLine:
                row.coordinator_contact_line || row.settings?.coordinator_contact_line || '',
            contactEmail:
                row.coordinator_email || row.settings?.coordinator_email || row.contact_email || '',
        };
    });
}
...
async function ensureSurveyRaw() {
    ...
    _rawFetchPromise = (async () => {
        try {
            ...
            if (error) {
                console.warn('ensureSurveyRaw:', error.message);
...
async function fetchAllSurveyRaw() {
    ...
    if (rawTotal === 0) return { data: [], error: null };

    // C1 fix: apply cap so analytics page can't OOM on very large datasets
    const total = Math.min(rawTotal, SURVEY_FETCH_CAP);
    if (rawTotal > SURVEY_FETCH_CAP) {
        console.warn(
            `[fetchAllSurveyRaw] dataset ${rawTotal} rows exceeds cap ${SURVEY_FETCH_CAP}; fetching newest ${SURVEY_FETCH_CAP} only`
        );
    }
    ...
    // C1 fix: batched concurrency instead of unbounded Promise.all
    const pageResults = await _batchedFetch(
        ranges.map(
            ([from, to]) =>
                () =>
                    sb
...
async function fetchAllSurveyResponses() {
    ...
    // C1 fix: cap total rows to prevent memory spike; show warning in status bar
    const total = Math.min(rawTotal, SURVEY_FETCH_CAP);
    if (rawTotal > SURVEY_FETCH_CAP) {
        console.warn(
            `[fetchAllSurveyResponses] dataset ${rawTotal} rows exceeds cap ${SURVEY_FETCH_CAP}; fetching newest ${SURVEY_FETCH_CAP} only`
        );
        const statusEl = document.getElementById('connection-status');
        if (statusEl) {
            statusEl.textContent = `โหลด ${SURVEY_FETCH_CAP.toLocaleString()} จาก ${rawTotal.toLocaleString()} รายการ (ถึง cap)`;
...
// Audit C1 fix: cap total rows fetched to prevent memory spike on large datasets.
// 15 orgs × ~500 responses/org = ~7,500 max; cap at 10,000 to be safe.
// If production exceeds this, migrate to server-side aggregation via RPC.
const SURVEY_FETCH_CAP = 10000;

// Max concurrent Supabase page-requests. Reduces burst load on Thai networks.
const _FETCH_CONCURRENCY = 3;

// Run an array of async factory functions in batches of `concurrency` at a time.
async function _batchedFetch(factories, concurrency = _FETCH_CONCURRENCY) {
    const results = [];
...
function _showLoadProgress(pct) {
...
function cancelLoad() {
    ...
    _hideLoadProgress();
    const statusEl = document.getElementById('connection-status');
    if (statusEl) { statusEl.textContent = 'ยกเลิกแล้ว'; statusEl.style.color = 'var(--W)'; }
    showToast('ยกเลิกการโหลดข้อมูลแล้ว', 'warn');
}
...
function _withTimeout(promise, ms = _LOAD_TIMEOUT_MS, label = 'เชื่อมต่อ') {
    ...
    return Promise.race([
        ...
        new Promise((_, reject) =>
            setTimeout(() => reject(new Error(`หมดเวลา${label} (${Math.round(ms / 1000)}s)`)), ms)
        ),
...
async function loadBackendCore(retryCount = 0) {
    if (retryCount === 0) { _loadAbort = new AbortController(); _showLoadProgress(5); }
    try {
        ...
        const [surveyRes, ch1LiteRes, usersRows, orgRows] = await _withTimeout(
            Promise.all([
        ...
        if (surveyRes.error)
            console.warn('loadBackendCore survey_responses:', surveyRes.error.message);
        if (ch1LiteRes.error)
            console.warn('loadBackendCore hrd_ch1_responses:', ch1LiteRes.error.message);
...
async function loadBackendExtras(retryCount = 0) {
    try {
        ...
        const [ch1FullRes, linksRes, orgHrCredRows, rawRes] = await _withTimeout(
            Promise.all([
                sb
                    .from('hrd_ch1_responses')
        ...
        if (ch1FullRes.error)
            console.warn('loadBackendExtras hrd_ch1_responses:', ch1FullRes.error.message);
        if (linksRes.error)
            console.warn('loadBackendExtras org_form_links:', linksRes.error.message);
        ...
        _showLoadProgress(100);
        setTimeout(_hideLoadProgress, 600);
        ...
        if (retryCount < _LOAD_MAX_RETRIES) {
            console.warn(
                `loadBackendExtras: ลองใหม่ครั้งที่ ${retryCount + 1}/${_LOAD_MAX_RETRIES}`
            );
            await new Promise(resolve => setTimeout(resolve, 800));
...
async function fetchDashboardSummary({ trendDays = 14 } = {}) {
    ...
    if (error) {
        console.warn('fetchDashboardSummary:', error.message);
        return null;
    }
...
function primeDashboardSummary(onFresh) {
    ...
    if (skipFetch) {
        try {
            if (typeof onFresh === 'function') onFresh(cached.data);
        } catch (e) {
            console.error(e);
        }
...
function summarizeOrgs() {
    ...
    if (_ch1Unmatched > 0) {
        console.warn(
            `[summarizeOrgs] ${_ch1Unmatched} of ${safeCh1Rows.length} ch1 rows could not be matched to a canonical org (check org_code and organization fields)`
        );
    }
...
function populateOrgSelects() {
    ...
    ids.forEach(id => {
        ...
        if (!el) return;
        const cur = el.value;
        el.innerHTML = `<option value="">ทุกองค์กร</option>${getOrgCatalog()
            .map(org => `<option value="${esc(org.name)}">${esc(org.name)}</option>`)
            .join('')}`;
        el.value = cur;
    });
    const ch1Sel = document.getElementById('ch1-link-org');
    if (ch1Sel) {
        ch1Sel.innerHTML = `<option value="">-- เลือกองค์กร --</option>${getOrgCatalog()
            .map(org => `<option value="${esc(org.name)}">${esc(org.name)}</option>`)
            .join('')}`;
    }
