...
const testData = [
    {
        ...
        strategic_overview: 'มุ่งเน้นการพัฒนาระบบสาธารณสุขให้ครอบคลุมทั่วถึง พร้อมยกระดับคุณภาพการบริการ',
        ...
        service_26_30: 200,
        service_over30: 100,
        sick_leave_days: 15000,
        sick_leave_avg: 3.0,
        clinic_users_per_year: 2000,
    ...
    },
    {
        ...
        service_26_30: 150,
        service_over30: 50,
        sick_leave_days: 12000,
        sick_leave_avg: 3.4,
        clinic_users_per_year: 1500,
    ...
    },
    {
        ...
        service_26_30: 600,
        service_over30: 100,
        sick_leave_days: 45000,
        sick_leave_avg: 3.0,
        clinic_users_per_year: 8000,
    ...
    },
    {
        ...
        service_26_30: 80,
        service_over30: 20,
        sick_leave_days: 8000,
        sick_leave_avg: 3.2,
        clinic_users_per_year: 3000,
    ...
    },
    {
        ...
        service_26_30: 20,
        service_over30: 0,
        sick_leave_days: 5500,
        sick_leave_avg: 3.1,
        clinic_users_per_year: 500,
    ...
    },
    {
        ...
        service_26_30: 10,
        service_over30: 0,
        sick_leave_days: 3600,
        sick_leave_avg: 3.0,
        clinic_users_per_year: 200,
    ...
    },
    {
        ...
        service_26_30: 10,
        service_over30: 0,
        sick_leave_days: 2400,
        sick_leave_avg: 3.0,
        clinic_users_per_year: 300,
    ...
    },
    {
        ...
        service_26_30: 20,
        service_over30: 0,
        sick_leave_days: 1800,
        sick_leave_avg: 3.0,
        clinic_users_per_year: 400,
    ...
    },
    {
        ...
        service_26_30: 40,
        service_over30: 10,
        sick_leave_days: 9000,
        sick_leave_avg: 3.0,
        clinic_users_per_year: 5000,
    ...
    },
    {
        ...
        service_26_30: 80,
        service_over30: 20,
        sick_leave_days: 13500,
        sick_leave_avg: 3.0,
        clinic_users_per_year: 1000,
...
async function runFormTest() {
    ...
    for (let i = 0; i < testData.length; i++) {
        ...
        try {
            ...
            await page.waitForSelector('.form-step[data-step="2"]', { state: 'visible' });
            await page.waitForTimeout(1000);
            
            // Check if sick_leave_days is visible, if not try alternative approach
            const sickLeaveInput = await page.locator('#sick_leave_days').first();
            await sickLeaveInput.waitFor({ state: 'visible', timeout: 10000 });
            await sickLeaveInput.fill(data.sick_leave_days.toString());
            
            await page.fill('#sick_leave_avg', data.sick_leave_avg.toString());
            await page.fill('#clinic_users_per_year', data.clinic_users_per_year.toString());
            await page.fill('#clinic_top_symptoms', 'ปวดหัว, ปวดกล้ามเนื้อ, ความดันสูง');
            await page.fill('#clinic_top_medications', 'พาราเซตามอล, ยาลดความดัน');
            
            // Mental health
            await page.fill('#mental_stress', 'พบผู้มีความเครียดสูง 5%');
            await page.fill('#mental_anxiety', 'พบภาวะวิตกกังวล 3%');
            await page.fill('#mental_sleep', 'มีปัญหาการนอนหลับ 8%');
            await page.fill('#mental_burnout', 'อาการหมดไฟ 2%');
            await page.fill('#mental_depression', 'ภาวะซึมเศร้า 1%');
            
            // Engagement
            await page.fill('#engagement_score', '2567: 75/100, 2566: 72/100');
            await page.fill('#engagement_low_areas', 'การสื่อสารและการมองโอกาส');
            await page.fill('#other_wellbeing_surveys', 'ทำแบบสำรวจความสุขภาพประจำปี');
            
            await page.click('button:has-text("ถัดไป")');
            await page.waitForTimeout(2000);
            
            // Step 3: Support systems
            await page.waitForSelector('.form-step[data-step="3"]', { state: 'visible' });
            await page.waitForTimeout(1000);
            
            // Click support systems radio buttons (select "มีตามแผน" for all)
            const supportRadios = await page.$$('input[name^="support_system"]');
            for (let j = 0; j < Math.min(3, supportRadios.length); j++) {
                await supportRadios[j].click();
            }
            
            await page.fill('#training_hours', '2568: 40 ชม., 2567: 35 ชม.');
            
            // Digital systems (check multiple)
            await page.check('input[value="e_doc"]');
            await page.check('input[value="e_sign"]');
            await page.check('input[value="cloud"]');
            await page.check('input[value="hr_digital"]');
            
            // Ergonomics
            await page.check('input[value="done"]');
            await page.fill('#ergonomics_done_detail', 'ดำเนินการแล้วทุกแผนก มีรายงานผล');
            
            await page.click('button:has-text("ถัดไป")');
            await page.waitForTimeout(2000);
            
            // Step 4: Direction and goals
            await page.waitForSelector('.form-step[data-step="4"]', { state: 'visible' });
            await page.waitForTimeout(1000);
            await page.waitForSelector('#rankList');
            
            // Select ranking items (click first 3 items)
            ...
            await page.fill('#hrd_plan_results', data.hrd_results);
            
            await page.click('button:has-text("ถัดไป")');
            await page.waitForTimeout(2000);
            
            // Step 5: Submit
            await page.waitForSelector('.form-step[data-step="5"]', { state: 'visible' });
            await page.waitForTimeout(1000);
            await page.waitForSelector('button:has-text("ส่งข้อมูล")');
